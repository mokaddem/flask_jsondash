(function(factory) {
    ...
    (function($) {
        ...
        var TreeFromJson = function(container, data, options) {
            ...
            this.container.addClass('treeFromJson');
        ...
        TreeFromJson.prototype = {
                ...
                if (clicked.data()[0].children === undefined) { // is leaf
                    resCircle = that.svg.selectAll(".node circle")
                        .filter(function(d) {
                            ...
                            let temp_inst = that.instructions.labels[il_last];
                            var labelIsManual = temp_inst !== undefined && temp_inst != 'l';
            ...
            adjust_instruction: function() {
                ...
                if (d !== undefined && d.length > 0) {
                    var end_i = adjustedInstructions[d_keyname].length-1;
                    var last_i = adjustedInstructions[d_keyname][end_i];
                    last_i = last_i.split(',');
                    last_i = last_i.length == 2 ? last_i[1] : last_i[0];
                    if (last_i == 'l') {
                        adjustedInstructions[d_keyname][end_i+1] = '';
                    }
                }
